...
So after you run `/todo add`, expect to see something like:
```
...
```
This is a one-way sync, not a live view - editing a todo or a progress
report (or deleting a todo) after it's already been posted to the thread
doesn't change or remove that post.

## `/todo` - your task list for the day

- **`/todo add`** - opens a form: **Task** (required, short), **Notes**
  ...
  against anything. Submit the form once per todo - call `/todo add`
...
- **`/progress add task:<...> status:<...>`** - two options, then a form:
  ...
  Posting progress against a given todo more than once is fine - if you
  reported Blocked earlier and you're now Done, just run `/progress add`
  against the same task again. Both reports are kept, so the tech lead
  sees the progression. To *correct* a report rather than add to it, use
  `/progress edit`.

- **`/progress edit report:<...> [status:<...>]`** - the `report` field
  autocompletes over today's own progress reports (shown as
  `Task — Status`). Opens the same form pre-filled with what you
  reported, so you can fix a mistake. `status` is optional here - leave
  it off to keep the current status, or set it to change it. This edits
  the existing report in place; it does **not** post a fresh message to
  the standup thread (the thread keeps whatever was synced originally).
...
- **`/progress help`** - a quick in-Discord reminder of the above.
...
```
/progress add task:"Write tests" status:Done
...
```
/progress add task:"Fixed a prod outage" status:Blocked
