...
use serenity::all::{
    Client, Context as SerenityContext, CreateCommand, CreateInteractionResponse,
...
impl EventHandler for Handler {
    async fn ready(&self, ctx: SerenityContext, ready: Ready) {
        ...
        let commands =
            vec![CreateCommand::new("ping").description("Check that dispatchd is alive")];
    ...
    async fn interaction_create(&self, ctx: SerenityContext, interaction: Interaction) {
        ...
        if command.data.name == "ping" {
            let reply =
                CreateInteractionResponseMessage::new().content("pong! dispatchd is alive.");
...
pub async fn run(token: String, guild_id: u64) -> Result<()> {
